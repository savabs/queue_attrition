...
def main() -> None:
    ...
    for _, row in a.iterrows():
        ...
        by = row.get("Proposed Completion Date")
        if pd.isna(by) or not str(by).strip():
            by = f"{int(row['queue_year']) + 8}-12-31"  # ISO study cycles run long
        R.log(
